<template>
  <header class="header">
    ...
    <div class="menu-bar">
      <ul class="nav">
        <li
          v-for="(item, i) in isMobile ? mobileNav : pcNav"
          :key="i"
          @click="emits('changeMenu', item.pages[0])"
          :class="{ active: item.pages.includes(currentPage) }"
        >
          {{ $t(item.label) }}
        </li>
      </ul>
...
const initLang = () => {
  ...
  const matchedLang = langList.find((item) => item.value === langValue);
...
.header {
  ...
  .menu-bar {
    ...
    .link-btn {
      ...
      border-radius: 50px;
      box-shadow: 0 0 3px 2px #8a6f12, 0 0 7px 5px rgba(116, 80, 25, 0.7),
        0 0 3px 2px #8a6f12 inset, 0 0 7px 5px rgba(116, 80, 25, 0.7) inset;
